...
def svc():
    # Загружаем модель ONNX при инициализации сервиса
    svc.session = ort.InferenceSession("/content/yolov8n_SGD.onnx")
    svc.input_name = svc.session.get_inputs()[0].name
    svc.output_name = svc.session.get_outputs()[0].name
    ...
    def predict(input_image) -> dict:
        ...
        predictions = svc.session.run([svc.output_name], {svc.input_name: image_array})
    ...
    # Возвращаем сервис с определённым API
    return svc
